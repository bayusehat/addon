...
class CombatChurnController extends Controller
{
    ...
    public function loadDataNew(Request $request)
    {
        ...
        $response['data'] = [];
        $query = DB::select("select a.*, case when b.nd_internet is not null then 'visited' else 'not visited' end status
        from(
        ...
        foreach ($query as $i => $v) {
            ...
            $response['data'][] = [
                ...
                $status,
                '
                <a href="'.url('cc/cn/detail/'.$v->id).'" class="btn btn-primary btn-block"><i class="fas fa-edit"></i> Edit</a>
                <a href="javascript:void(0)" class="btn btn-danger btn-sm" onclick="deleteForm('.$v->id.')"><i class="fas fa-trash"></i> Hapus</a>
                '
            ];
        }
    ...
    {
        $response['data'] = [];
        $q = DB::select("select c.area witel, sum(case when flag = 'caps' then 1 else 0 end) visit_caps, sum(case when flag = 'ct0' then 1 else 0 end) visit_ct0 
            from cc_form a 
        ...
        foreach ($q as $v) {
            $response['data'][] = [
                ...
                $v->visit_ct0,
                '
                <a href="javascript:void(0)" class="btn btn-primary"><i class="fas fa-download"></i> Download</a>
                '
            ];
        }
